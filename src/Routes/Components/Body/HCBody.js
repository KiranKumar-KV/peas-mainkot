...
export default class HCBody extends PureComponent 
 {
    constructor(props)
    {
        ...
        this.state={
            ...
            floorLength:0,
            allStatus:[],
            newStatus:["Ordered","Served","Empty","Billed","takeaway"],
            newFloor:[],
            allFloors:[],
            Ordered:{
                value:"Ordered",
    ...
    {
        if(this.state.route_type!=this.props.type)
        {
           await this.setState({
                route_type:this.props.type
            })
            // await this.props.routePass(this.props.type)
            await this.get();
        }
    }
    async get()
    {
        try
        {
            ...
            {
                 data=await axios.get(API_URL+'/hms/kot/tableview')
            }
            else if(this.props.type=='takeaway')
            {
                data=await axios.get(API_URL+'/hms/kot/tableviewTakeaway');
               
            }
            if(data.status===200)
            {
                tables=await data.data;
                employee_data=await axios.get(API_URL+'/hms/kot/ViewEmployee')
                
            }
            let options=[];
            await employee_data.data.map(data=>{
                options.push({value:data.employee_id,label:data.employee_name})
            })
            let avail_room=[];
            tables= await tables.filter(tables=>{
   ...
   componentWillMount()
    {
        this.get();
        this.setState({
            allStatus:[this.state.Ordered,this.state.Served,this.state.Empty,this.state.Billed,this.state.takeaway]
        })
    }
    ...
    addEmployeeRender()
    {
        if(this.props.type!=='takeaway')
        {
                ...
                return(
                    <div>
                        <Icon className="addEmployee" 
                            size={30} 
                            onClick={this.handleEmployee.bind(this)} 
                            icon={ic_person_add} 
                        />
                    </div>
                )
            ...
            else
            {
                return(
                    <div>
                        <Icon className="addEmployee upload" 
                            size={30}  
                            onClick={this.handleEmployee.bind(this)} 
                            icon={checkCircle} 
                        />
    ...
    render() {
        ...
        return (
            <div>
                ...
                <div>
                    <HCHeader addEmployee={this.state.addEmployee} 
                        selectOptions={this.state.selectOptions} 
                        handleSelect={this.handleSelect.bind(this)} 
                        handleStatus={this.handleStatus.bind(this)} 
                        handleFloors={this.handleFloors.bind(this)} 
